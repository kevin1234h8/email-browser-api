...
";
            ...
            try
            {
                using (MemoryStream storedStream = new MemoryStream(content))
                {
                    ...
                    if (message.BodyHtml != null)
                    {
                        ...
                        HtmlResponse = $"<html><head><meta charset=\"utf-8\"></head><body>{templateHeader}{bodyHtml}{attachmentsHtml}</body></html>";
                    }
                    else if (message.BodyText != null)
                    {
                        ...
                        // Construct the HTML response
                        HtmlResponse = $"<html><head><meta charset=\"utf-8\"></head><body>{templateHeader}{bodyText}{attachmentsHtml}</body></html>";
                    }
                    else
                    {
        ...
        private static string HeaderTemplate(string from, string sentDate, string to, string receivedDate, string cc, string subject)
        {
            ...
            string name = nameWithQuotes.Replace("\"", "");
      ...
      extractedName +
        ...
        <div style='display: flex; justify-content: space-between; gap: 1rem'>
          <div style='width: 75%;'>
            ...
            HeaderTemplate += @"</div>
          <div class='send-date' style='font-size: 11px;'>" + sentDate + @"</div>
        </div>
    </div>
  </div>
<hr />
